...
//        .setMaster("local[2]")
      ...
      val groupId = "fgwfdfafa"

      //获取jt8080数据
      var recordInputStream: InputDStream[ConsumerRecord[String, String]] = MyKafkaUtil.getKafkaStream(topic, ssc, groupId)
      val transportInputStream: DStream[DataBusBean] = recordInputStream.flatMap { record =>
        val returnSeq = ArrayBuffer.empty[DataBusBean]
        try {
...
//
      ...
      //过滤掉服务费到期车辆
      val filterServiceFeeStream: DStream[DataBusBean] = joinVehicleStream.mapPartitions { dataBusBean =>
        val dataBusBeanList: List[DataBusBean] = dataBusBean.toList
